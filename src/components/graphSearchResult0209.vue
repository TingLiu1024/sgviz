...
export default {
  ...
  methods: {
    ...
    plotOneStampFdg(dynamicGroup, tsp, data) {
      ...
      let scaleFdgX = d3
          .scaleLinear()
          .domain([mapCenter[0] - mapRange, mapCenter[0] + mapRange])
          .range([0 + cur.sgMargin, cur.rectH - cur.sgMargin]),
        scaleFdgY = d3
          .scaleLinear()
          .domain([mapCenter[1] - mapRange, mapCenter[1] + mapRange])
  ...
  watch: {
    plotFlag: function () {
      if (this.plotFlag) {
            ...
            cur.searchMatchResult.forEach((logRes) => {
              ...
              logRes["frames"].forEach((frm) => {
                ...
                cur.plotOneStampFdg(
                  dynamicGroup,
                  frm,
                  cur.searchSgData[logId_]
                );
              });
            });
          } else if (cur.brushViewState) {
            console.log("plot brush result!");
            console.log(cur.searchMatchResult);
            cur.searchMatchResult.forEach((logRes) => {
              let logId_ = logRes["logId"];
              logRes["frames"].forEach((frm) => {
                let dynamicGroup = d3.select(
        ...
        cur.$store.commit('closePlot')
      }
    },
    // brushViewState:function(){
    //   if(this.brushViewState){

    //     let cur = this

    //     cur.sleep(1000).then(() => { console.log("plot brush result!");
    //     console.log(cur.projectionSgData)
    //     cur.projectionBrushData.forEach((logRes)=>{
    //     let logId_ = logRes["logId"]
    //     logRes["frames"].forEach(frm =>{
    //       let dynamicGroup = d3.select("#svg" + logId_ + '_' + frm.toString())
    //       // 加svg前缀原因 ： 数字开头会导致selector失败
    //       // let dynamicGroup = document.getElementById(logId_ + '_' + frm)
    //       // let dynamicGroup = d3.select('#3138907e-1f8a-362f-8f3d-773f795a0d01_1')
