<template>
  <div id="visualSummaryView" :style="divCss">
    ...
    <svg
      id="svg"
      :width="width"
      :height="lineChartHeight + streamHeight + marginTop * 3"
    >
      ...
      <g
        ...
        id="egoCar"
        :transform="`translate(${
          marginLeft + timeScale(parseInt(currentTime))
        }, ${marginTop + yScale(yawList[parseInt(currentTime)])})`"
      >
      ...
      <g
        id="lineLegend"
        :transform="`translate(${marginLeft + 30},${marginTop})`"
      >
...
export default {
  ...
  data() {
    return {
      ...
      NotTurnColor: "#b3b3b3",
      // 加速度异常
      accChangeThreshold: 10,
      accCircleMax:5,
      accCircleMinRatio:0.3,
      // 加速度异常
      legendW: 15,
      legendH: 15,
      legendInterVal: 10,
      ...
      streamColorDict: {
        PEOPLE_notOnLane: "#FFCDBF",
        PEOPLE_onLane: "#FF8680",
        VEHICLE_onLane: "#1f78b4",
        VEHICLE_notOnLane: "#a6cee3",
        PEOPLE: "#D25565",
        VEHICLE: "#3E6D9C",
        PEOPLE_Free: "#FFCDBF",
        VEHICLE_Free: "#a6cee3",
  ...
  computed: {
    ...mapState([
      ...
      "currentEventData",
      "speedFillColor",
      "accelerationColor","legendFontSize"
    ]),
    divCss() {
      return (
  ...
  methods: {
    plotLineChart() {
      ...
      legendG
        .selectAll(".lineText")
        .data(["Speed", "Left Turn", "Right Turn", "Straight", "Intersection"])
        .enter()
        .append("text")
        .attr("y", 14)
        .attr("x", (d, i) => {
          return 40 + i * (cur.lengedXInterval );
        })
        .text((d) => d)
      ...
      let scaleX = this.timeScale,
        scaleY = this.yScale;
      let egoTrackingDataSorted = d3.filter(
        this.currentTrackingData["trackingInfos"],
        (d) => d.track_label_uuid == "ego"
      );
      // 计算加速度异常点
      window.egoData = egoTrackingDataSorted;
      // console.log(egoTrackingDataSorted)
      let accGapMax = cur.accChangeThreshold
      let accAbnormalPoints = egoTrackingDataSorted.reduce(function (
        prev,
        current,
        index
      ) {
        // console.log(index)
        if (index >= 1) {
          let prevAcc = egoTrackingDataSorted[index - 1].acceleration;
          let curAcc = current.acceleration;

          if ((prevAcc != null) & (curAcc != null)) {
            let accGap_ = Math.abs(curAcc - prevAcc)
            if ( accGap_ >= cur.accChangeThreshold) {
              // console.log(Math.abs(curAcc - prevAcc))
              // prev.push([current.frame, prevAcc, curAcc])
              accGapMax = Math.max(accGapMax, accGap_)
              prev.push({
                frame: current.frame,
                prevAcc: prevAcc,
                currentAcc: curAcc,
      ...
      []);
      console.log("acc", accAbnormalPoints);
      ...
      cutoffs.forEach(function ([start, end]) {
        ...
        tmp["oriData"].forEach(function (d) {
          ...
          speedLowers.push([
            scaleX(d.frame),
            scaleY(d.yawMod + cur.lineHalfWidth) - 5,
          ]);
          if (d.speed == null) {
            speedUppers.push([
              scaleX(d.frame),
            ...
            speedUppers.push([
              scaleX(d.frame),
              scaleY(d.yawMod + cur.lineHalfWidth) - scaleSpeed(d.speed) - 5,
            ]);
          }
        ...
        if (currentLane) {
          tmp["turn_direction"] =
            mapData["laneInfos"][currentLane.toString()]["turn_direction"];
          tmp["is_intersection"] =
            mapData["laneInfos"][currentLane.toString()]["is_intersection"];
        } else {
      ...
      // speedLowers.pop()
      //   speedUppers.pop()
      // accAbnormalPoints的y值
      accAbnormalPoints.forEach((d) => {
        Object.assign(d, { location: speedUppers[d.frame] });
      });

      console.log(accAbnormalPoints);
      ...
      linesGroup
        ...
        .attr("d", (d) =>
          d3.line()(
            d3.map(d.plotPoints, function (dd) {
        ...
        .attr("stroke", (d) =>
          d.is_intersection ? this.intersectionStrokeColor : "none"
        )
        ...
        .on("mousemove", (event) => {
          ...
          let curYaw = !cur.yawList[currentT]
            ? null
            : cur.yawList[currentT].toFixed(2);
          cur.tooltipContent =
            "Yaw" + " : " + curYaw + "<br>" + "Timestamp : " + currentT;
        })
        .on("mouseout", () => {
      ...
      linesGroup
        ...
        .on("mousemove", (event) => {
          ...
          let curSpeed = !cur.speedList[currentT]
            ? null
            : cur.speedList[currentT].toFixed(2);
          cur.tooltipContent =
            "Speed" + " : " + curSpeed + "<br>" + "Timestamp : " + currentT;
        })
        .on("mouseout", () => {
          tooltip.css("display", "none");
        });
      let accScale = d3.scaleLinear()
        .domain([cur.accChangeThreshold, accGapMax])
        .range([cur.accCircleMinRatio * cur.accCircleMax, cur.accCircleMax]);
      linesGroup
        .append("g")
        .selectAll(".accAbnormalCircle")
        .data(accAbnormalPoints)
        .enter()
        .append("circle")
        .attr("cx", (d) => d.location[0])
        .attr("cy", (d) => d.location[1])
        .attr("r", (d) => accScale(d.accGap))
        
        .attr("fill", cur.accelerationColor)
        .attr('stroke-width', cur.accCircleMax * 0.2)
        .attr("stroke", "red")
        
        // .attr("stroke", cur.accelerationColor)
        // .attr("fill", "none")
        ...
        .on("mousemove", (event, d) => {
          ...
          cur.tooltipContent =
            "current acceleration" + " : " + d.currentAcc.toFixed(2) + "<br>" + 
            "previous acceleration" + " : " + d.prevAcc.toFixed(2) + "<br>" +
            "Timestamp : " + d.frame;
        })
        .on("mouseout", () => {
          tooltip.css("display", "none");
    ...
    getStreamGraphData(data) {
      //处理数据
      const objectTracking = d3.filter(
        data,
        (d) => d.track_label_uuid != "ego"
      );
      ...
      const groupCountData = d3.rollups(
        objectTracking,
        (v) => v.length,
        (d) => d.frame,
        (d) => d.label_class,
      ...
      groupCountData.forEach(function (d) {
        ...
        keys.forEach(function (key) {
          ...
          tmpDict["VEHICLE"] =
            tmpDict["VEHICLE_onLane"] + tmpDict["VEHICLE_notOnLane"];
          tmpDict["PEOPLE"] =
            tmpDict["PEOPLE_onLane"] + tmpDict["PEOPLE_notOnLane"];
        });
        res.push(tmpDict);
      });
      let stackedData = d3
    ...
    plotStreamGraph() {
      ...
      const group = d3
        ...
        .attr(
          "transform",
          `translate(${cur.marginLeft}, ${
            cur.marginTop * 2 + cur.lineChartHeight
          })`
        );
      group.selectChildren().remove();

      let countData = this.getStreamGraphData(
        this.currentTrackingData["trackingInfos"]
      );
      window.countData = countData;
      let stackedData = countData["stack"],
        res = countData["res"];
      ...
      let mouseover = function (event) {
        ...
        d3.selectAll(".streamArea").style("opacity", 0.2);
        d3.select(this)
          //.style("stroke", "black")
          .style("opacity", 1);
      };
      ...
      group
        ...
        .style("fill", function (d) {
          return cur.streamColorDict[d.key];
        })
        .attr("d", area)
        .attr("transform", `translate(${0},${0})`)
      ...
      legendG
        ...
        .attr(
          "y",
          (d, i) =>
            cur.streamHeight - (4 - i) * (cur.legendW + cur.legendInterVal)
        )
        .attr("x", cur.marginLeft)
        .attr("fill", (d) => cur.streamColorDict[d]);
      legendG
        .selectAll(".streamText")
        .data(legendText)
        .enter()
        .append("text")
        .attr("x", 40 + cur.marginLeft)
        .attr("y", (d, i) => {
          return (
            15 + cur.streamHeight - (4 - i) * (cur.legendW + cur.legendInterVal)
          );
        })
        .text((d) => d)
        .attr("text-anchor", "left")
    ...
    plotEventheatmap() {
      ...
      const group = d3
        ...
        .attr(
          "transform",
          `translate(${cur.marginLeft}, ${
            cur.marginTop + cur.lineChartHeight + cur.streamHeight
          })`
        );
      group.selectChildren().remove();
      ...
      const rectHScale = d3
        .scaleLinear()
        .domain(d3.extent(d3.map(heatmapData, (d) => d.count)))
      ...
      group
        ...
        .attr("y", (d) =>
          d.label_class == "VEHICLE"
            ? heatmapYscale(d.event)
            : rectH * 7 + heatmapYscale(d.event)
        )
        ...
        .attr("stroke-dasharray", (d) => {
          if (d.event.indexOf("laneObject") != -1) {
            ...
            return (
              "0," + rectHScale(d.count) + "," + rectHScale(d.count) + ", 0"
            );
            ...
            return (
              "0," +
              rectHScale(d.count) / 4 +
              "," +
              rectHScale(d.count) / 2 +
              "," +
              rectHScale(d.count) / 4
            );
          }
